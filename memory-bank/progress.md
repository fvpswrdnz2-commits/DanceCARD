...
- Diagnosed the initial simulator request failure as WeChat Developer Tools legal-domain enforcement, used a temporary local bypass to confirm the backend, then registered the development CloudBase gateway as the Mini Program request legal domain and re-enabled domain checks.
...
- Pushed the compatibility fix to public `origin/main` as `41ebf32` and applied it successfully to the CloudBase development environment. Milestone 3 and milestone 5 SQL suites pass after deployment.
- Replayed the released 1.x client queries through the CloudBase browser SDK: Beijing returns `全市 → 嘉禾舞社`; Shanghai returns `全市 → CASTER舞蹈教室 / GH5 DANCE STUDIO`. Live V2 H5 verification still opens Beijing directly to `选择舞室`, confirming that the temporary backend bridge does not restore a district step in V2.
