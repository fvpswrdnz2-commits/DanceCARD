...
- 产品路线：V2 正式发布移动端 H5；微信端保持可构建并通过开发者工具冒烟测试
...
V2 暂不引入 Tailwind。舞卡页面结构简单，而 Tailwind 在小程序端需要额外转换和兼容配置，当前收益不足以抵消复杂度。
...
V2 不实现微信登录，不提交审核、不发布且不要求真机验收。只保证微信小程序构建成功，并在微信开发者工具中完成页面级冒烟测试。
...
V2 允许游客直接执行单条联系方式获取，不增加登录、验证码或频率限制；接口仍必须只接受单个公开有效次卡 ID，并再次检查用户、城市、舞室和次卡状态。
...
- 城市管理。
- 城市级舞室添加、分店信息编辑和同城重复检查。
- 次卡检索、隐藏和删除。
- 用户查看和禁用。
- 管理员操作记录。

独立的基础管理后台属于 V2 必做范围。CloudBase 控制台只用于首次设置管理员、环境配置和紧急运维，不作为日常内容管理界面。
...
从项目早期开始定期执行微信小程序构建，并在微信开发者工具中完成关键页面冒烟测试。V2 不要求微信登录、提交审核、发布或真机验收。
...
V2 不采用，避免增加小程序端样式转换和兼容配置。

### Redux

V2 状态规模较小，不需要引入较重的全局状态方案。

### 自建服务器

V2 不部署和维护传统服务器，使用 CloudBase Serverless 能力降低运维成本。
...
这套方案作为舞卡 V2 的正式技术选型。后续技术设计、数据库设计和项目初始化均以此文档为依据。
